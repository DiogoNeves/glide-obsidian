...
Before choosing the daily output, run `$glide-update-user-profile`, then scan `Agent HQ/Open Loops.md`, active goals, questions, ponders, contradictions, recent decisions, and relevant area files. When configured, also check relevant inbox, calendar, task, newsletter, or app-action sources. When several daily focus areas are plausible, explore each candidate narrowly, then integrate the findings and choose the smallest useful user-facing output.
