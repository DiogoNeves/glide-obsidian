...
Use the Glide skill as the source of truth for context loading, morning checks, daily-output selection, parallel focus scanning when supported, guardrails, and light evaluation.

Before choosing the daily output, scan `Agent HQ/User Profile.md`, `Agent HQ/Open Loops.md`, active goals, questions, ponders, contradictions, recent decisions, and relevant area files. When configured, also check relevant inbox, calendar, task, newsletter, or app-action sources. Refresh `User Profile.md` only when recent context contains durable signal; keep long-term updates conservative and let lately/right-now context move more often. When several daily focus areas are plausible, explore each candidate narrowly, then integrate the findings and choose the smallest useful user-facing output.
